...
./dbgen -s 0.1 = create 0.1gb of data
./dbgen -s 1 -y = create 0.1gb of data



if you do ls -lrt, you see that reguin.tbl, nation.tbl, ... files are created. 

create a folder /data
copy these files into a /data folder and change to .csv
`mkdir -p ../../tpc-h/data && for f in *.tbl; do cp "$f" "tpc-h/data/${f%.tbl}.csv"; done`


#!/bin/bash
# Create the target directory if it doesn't exist
mkdir -p ../../tpc-h/data
mkdir -p ../../tpc-h/queries
mkdir -p ../../tpc-h/schema

# Loop through all .tbl files in the current directory
for f in *.tbl; do
  # Copy each .tbl file to the target directory with a .csv extension
  cp "$f" "../../tpc-h/data/${f%.tbl}.csv"
done

# Coopy all queries in queries-dir and copy to /tpc-h/queries
for q in 
